...
camera = new THREE.PerspectiveCamera(70, width / height, 0.0001, 1000);
camera.position.z = 2;
...
geometry = new THREE.PlaneGeometry(0.6, 0.4, 10, 10);
...
const images = [];
const images_num = 6;
let loaderfinal = new THREE.TextureLoader();
  ...
  for (let j = 0; j < 6; j++) {
    for (let i = 0; i < images_num; i++) {
      ...
      window.addEventListener('load',()=>{
        loaderfinal.load(`../assets/images/image0${(i % 6) + 1}.jpg`)
      })

      material.uniforms.uTexture.value = loaderfinal.load(
        `../assets/images/image0${(i % 6) + 1}.jpg`
      );
      ...
      lenis.on("scroll", () => {
        if (Math.abs(scroll.scrollVelocity) > 0.3) {
...
loader.load("../assets/ashfall.glb", function (gltf) {
  ...
  gltf.scene.children[0].children[0].material = new THREE.MeshBasicMaterial({
    color: "grey",
    wireframe: true,
  });
...
function animate(time) {
  ...
  if (intersects.length > 0) {
    ...
    if (INTERSECTED != intersects[0].object) {
      if (INTERSECTED) {
        gsap.to(INTERSECTED.material.uniforms.hover, {
          value: 0,
          duration: 0.3,
          ease: "circ",
        });
